...
export default function CategoriesEdit(props: EditProps<Attributes>) {
	const {
		attributes: {
			displayAsDropdown,
			showHierarchy,
	...
	const getCategoriesList = (parentId: string) => {
	...
	const toggleAttribute = (attributeName: string) => (newValue: any) =>
	...
	const TagName =
		!!categories?.length && !displayAsDropdown && !isResolving ? "ul" : "div";
	...
	return (
		<TagName {...blockProps}>
			...
			{!isResolving &&
				categories?.length > 0 &&
				(displayAsDropdown ? renderCategoryDropdown() : renderCategoryList())}
		</TagName>
	);
}
